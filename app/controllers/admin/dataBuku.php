...
class dataBuku extends controller {
	...
	public function index ($dataUrl = []) {
		...
		if ( isset($_SESSION["keyBuku"]) ) {
			$jm = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM buku WHERE no LIKE '%$key%' || judul LIKE '%$key%' || pengarang LIKE '%$key%' || gambarBuku LIKE '%$key%' || bukuDipinjam LIKE '%$key%' || jumlahBuku LIKE '%$key%' || sinopsis LIKE '%$key%' || jenisBuku LIKE '%$key%'"));
		}
